    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser? "max-md:hidden" : ""}`}>
      <div className='pb-5'>
        <div className='flex justify-between items-center'>
          <img src="./src/assets/buzzup.svg" alt="logo" className='max-w-10' />
        ...
        <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
          <img src="./src/assets/search_icon.png" alt="search" className='w-3' />
      ...
      <div className='flex flex-col'>
        {filteredUsers.map((user,index) => (
          <div key={index} onClick={() => {setSelectedUser(user); setUnseenMessages(prev => ({...prev, [user._id]: 0}))}} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${setSelectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
            <img src={user?.profilePic || asset.avatar} alt="" className='w-[35px] aspect-[1/1] rounded-full' />
